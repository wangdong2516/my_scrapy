...
class ExecutionEngine:
    def __init__(
        ...
        self.settings: Settings = crawler.settings  # 这里也把settings配置保存到引擎中
        self.signals: SignalManager = crawler.signals  # 信号
        assert crawler.logformatter
        self.logformatter: LogFormatter = crawler.logformatter
        self.slot: Slot | None = None
        self.spider: Spider | None = None  # 当前爬虫
        self.running: bool = False
        self.paused: bool = False
        self.scheduler_cls: type[BaseScheduler] = self._get_scheduler_class(
            crawler.settings
        )  # 从settings中找到SCHEDULER配置项指定的调度器，找到Scheduler类
        # 从settings中找到DOWNLOADER配置项指定的下载器，找到downloader类
        downloader_cls: type[Downloader] = load_object(self.settings["DOWNLOADER"])
        self.downloader: Downloader = downloader_cls(crawler)  # 实例化downloader类,默认是scrapy.core.downloader.Downloader
        self.scraper: Scraper = Scraper(crawler)  # 创建Scraper类,它是引擎连接爬虫类的桥梁
