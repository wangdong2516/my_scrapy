...
class Scheduler(BaseScheduler):
    ...
    def __init__(
        ...
        self.df: BaseDupeFilter = dupefilter  # 指纹过滤器(主要用来过滤重复请求)
        self.dqdir: str | None = self._dqdir(jobdir)  # 队列任务文件夹
        self.pqclass: type[ScrapyPriorityQueue] | None = pqclass  # 优先级任务队列类
        self.dqclass: type[BaseQueue] | None = dqclass  # 磁盘任务队列类
        self.mqclass: type[BaseQueue] | None = mqclass  # 内存任务队列类
        self.logunser: bool = logunser  # 日志是否序列化
    ...
    def from_crawler(cls, crawler: Crawler) -> Self:
        ...
        # 实例化指纹过滤器,从配置文件中依次获取优先级任务队列类、磁盘队列类、内存队列类
        return cls(
