...
class Crawler:
    def __init__(
        ...
        self.spidercls: type[Spider] = spidercls  # 自定义爬虫类
        self.settings: Settings = settings.copy()  # 复制配置对象
        self.spidercls.update_settings(self.settings)  # 更新配置
        self._update_root_log_handler()  # 更新日志配置
        ...
        self.request_fingerprinter: RequestFingerprinter | None = None  # 指纹
    ...
    def crawl(self, *args: Any, **kwargs: Any) -> Generator[Deferred[Any], Any, None]:
        ...
        try:
            # 实例化自定义爬虫类,主要是调用Spider类的from_crawler方法创建的
            self.spider = self._create_spider(*args, **kwargs)
            self._apply_settings()  # 应用配置项
            self._update_root_log_handler()  # 更新日志配置
            # 创建引擎,返回ExecutionEngine的实例对象,scheduler_cls属性是SCHEDULER配置项指定的(默认为SCHEDULER=scrapy.core.scheduler.Scheduler)
            self.engine = self._create_engine()
            # 执行自定义爬虫类(如果没有自定义start_request方法，则执行Spider类)中定义的start_request,获取到生成器的下一个元素
...
class CrawlerRunner:
    ...
    def _get_spider_loader(settings: BaseSettings) -> SpiderLoader:
        ...
        cls_path = settings.get("SPIDER_LOADER_CLASS")  # 爬虫加载类
        loader_cls = load_object(cls_path)  # 导入对应的模块
    ...
    def _create_crawler(self, spidercls: str | type[Spider]) -> Crawler:
        if isinstance(spidercls, str):  # 如果是字符串 则从spider_loader中加载这个爬虫类
