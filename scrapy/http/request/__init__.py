...
class Request(object_ref):
    ...
    def __init__(
        ...
        self._encoding: str = encoding  # this one has to be set first  编码
        self.method: str = str(method).upper()  # 请求方式
        self._set_url(url)  # 转义url，最终赋值给self._url
        self._set_body(body)  # 设置请求体，最终赋值给self._body
        if not isinstance(priority, int):
            raise TypeError(f"Request priority not an integer: {priority!r}")
        self.priority: int = priority  # 设置优先级

        if not (callable(callback) or callback is None):  # 检查callback是否存在并且是可调用的
            raise TypeError(
                f"callback must be a callable, got {type(callback).__name__}"
            )
        if not (callable(errback) or errback is None):
            raise TypeError(f"errback must be a callable, got {type(errback).__name__}")
        self.callback: CallbackT | None = callback  # 设置回调函数
        self.errback: Callable[[Failure], Any] | None = errback  # 设置异常回调函数

        self.cookies: CookiesT = cookies or {}  # 设置cookies
        self.headers: Headers = Headers(headers or {}, encoding=encoding)  # 设置请求头
        self.dont_filter: bool = dont_filter  # 是否需要过滤
